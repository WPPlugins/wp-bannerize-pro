...
use WPBannerize\WPBones\Container\Container;
use WPBannerize\WPBones\Database\WordPressOption;
use WPBannerize\WPBones\View\View;
use WPBannerize\WPBones\Contracts\Foundation\Plugin as PluginContract;
...
class Plugin extends Container implements PluginContract
{
  ...
  public function boot()
  {
     ...
     *
      ...
      Description = "WP Bannerize is a WP Bones boilperate plugin"
    ...
    // Load text domain
    load_plugin_textdomain( "wp-bannerize", false, trailingslashit( basename( $this->basePath ) ) . $this->DomainPath );
   ...
   *
   * @return \WPBannerize\WPBones\View\View
   */
        ...
        foreach ( $menu[ 'items' ] as $key => $subMenu ) {
          ...
          if ( isset( $subMenu[ 'route' ][ 'load' ] ) ) {
            ...
            $func = create_function( '', sprintf( '$instance = new %s; return $instance->%s();',
                                                  'WPBannerize\\Http\\Controllers\\' . $controller, $method ) );
          ...
          if ( isset( $subMenu[ 'route' ][ 'resource' ] ) ) {
            ...
            $func = create_function( '', sprintf( '$instance = new %s; if( method_exists( $instance, "load" ) ) { return $instance->load(); }',
                                                  'WPBannerize\\Http\\Controllers\\' . $controller ) );
            add_action( "load-{$subMenuHook}", $func );
          }
        }
      }

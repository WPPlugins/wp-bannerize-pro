...
class WPBannersQuery
{
  ...
  protected $defaultArgs = [
    'category'        => '',            // deprecated since 1.5.0 - user 'categories' instead
  ...
  public function select( $args = [] )
  {
    ...
    if ( ! empty( $this->args[ 'categories' ] ) ) {
      ...
      // Since 1.5.0 'categories' can be an array
      $categories_array = is_string( $this->args[ 'categories' ] ) ? explode( ',', $this->args[ 'categories' ] ) : (array) $this->args[ 'categories' ];

      // Prepare a sanitize array with terms id
      $categories = [];

      // Sanitize imput bannerize categories
  ...
  public function __toString()
  {
    ...
    if ( ! is_null( $this->posts ) ) {
      ...
      ?>
      <div class="wp_bannerize_container wp_bannerize_layout_<?php echo $this->args[ 'layout' ] ?>"><?php

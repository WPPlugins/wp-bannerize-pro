...
class WPBannerizePost
{
  ...
  public function __get( $name )
  {
    if ( $this->post ) {
      ...
      return get_post_meta( $this->post->ID, $metaKey, true );
  ...
  public static function where()
  {
    ...
    if ( func_num_args() > 2 ) {
      list( $field, $cond, $value ) = func_get_args();
    }
    ...
    switch ( $field ) {
      case 'path':
        ...
        break;
      case 'title':
        $instance->post = get_page_by_title( $value, OBJECT, $instance->postType );
        break;
    }
  ...
  public function display()
  {
    ...
    $classes = apply_filters( 'wp_bannerize_classes', $classes );
    ...
    $styles = implode( ';', $stack );

    ?>
    <div class="<?php echo implode( ' ', $classes ) ?>"
         style="<?php echo $styles ?>"
         data-title="<?php echo $this->post_title ?>"
         data-mime_type="<?php echo sanitize_title( $this->banner_mime_type ) ?>"
      <?php echo empty( wpbones_value( $this->banner_impressions_enabled ) ) ? '' : ' data-impressions_enabled="true"' ?>
      <?php echo empty( wpbones_value( $this->banner_clicks_enabled ) ) ? '' : ' data-clicks_enabled="true"'  ?>
         data-banner_id="<?php echo $this->ID ?>"
         id="wpbanner-<?php echo $this->ID ?>">
      <?php echo $content ?>
    </div>
    <?php
  }
  ...
  {
    // Prepare return
    $size = array( 0, 0, 'mime' => 'text/plain' );
